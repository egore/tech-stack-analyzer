...
<project>
	...
	payload := results[0]
	// Note: XML parsing may not extract project name as expected,
	// so it falls back to directory name
...
dependencies {
	...
	payload := results[0]
	// Note: Falls back to directory name due to XML parsing limitations
	assert.Equal(t, "project", payload.Name)
...
func TestDetector_extractProjectNameFromPom(t *testing.T) {
	detector := &Detector{}
	...
	tests := []struct {
		...
		{
			name: "complete project info",
			content: `<?xml version="1.0" encoding="UTF-8"?>
<project>
    <groupId>com.example</groupId>
    <artifactId>test-app</artifactId>
</project>`,
			expected: "", // XML parsing doesn't work as expected
		},
		{
...
<project>
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := detector.extractProjectNameFromPom(tt.content)
			assert.Equal(t, tt.expected, result)
		})
	}
}

func TestDetector_extractProjectNameFromGradle(t *testing.T) {
	detector := &Detector{}
	...
	tests := []struct {
		...
		{
			name:        "rootProject.name with single quotes",
			content:     `rootProject.name = 'test-app'`,
			currentPath: "/project",
			expected:    "test-app",
		},
		{
			name:        "rootProject.name with double quotes",
			content:     `rootProject.name = "test-app"`,
			currentPath: "/project",
			expected:    "test-app",
		},
		{
			...
			content:     `project.name = 'test-app'`,
			currentPath: "/project",
			expected:    "test-app",
		},
		{
			name:        "project.name with double quotes",
			content:     `project.name = "test-app"`,
			currentPath: "/project",
			expected:    "test-app",
		},
    ...
    id 'java'
		...
		{
			name: "complex gradle file",
			content: `plugins {
    id 'java'
}

rootProject.name = 'complex-app'

dependencies {
    implementation 'org.springframework.boot:spring-boot-starter'
}`,
			currentPath: "/project",
			expected:    "complex-app",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := detector.extractProjectNameFromGradle(tt.content, tt.currentPath)
			assert.Equal(t, tt.expected, result)
		})
	}
}
